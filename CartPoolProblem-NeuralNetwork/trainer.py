...
class Trainer:
    ...
    def train(self, episodes_number=100_000, interval=1000, cache_file_path=None):
        ...
        for i in range(episodes_number):
            reward = self.one_episode_train(policy=self.policy, q_value_model=self.q_value_model)
            ...
            if i % interval == 0 and i != 0:
                ...
                if avg_reward > max_reward:
                    ...
                    max_reward_q_model = copy.copy(self.q_value_model)
                print(f'Episode [{i}/{episodes_number}] Avarage cumulative reward = {avg_reward}  Epsilon = {self.policy.current_epsilon}')
                reward_sum = 0
